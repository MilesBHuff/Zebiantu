#!/usr/bin/env bash
function helptext {
    echo "Usage: format-sys.bash device0 [device1 ...]"
    echo
...
[[ -x "$ASHIFT_SCRIPT" ]] && ASHIFT=$("$ASHIFT_SCRIPT" $ENV_SECTOR_SIZE_SYS)
...
zpool create -f \
    ...
    -O keylocation="file:///etc/zfs/keys/$ENV_POOL_NAME_SYS.key" \
    \
    -O compression="$ENV_ZPOOL_COMPRESSION_BEST" \
    \
    -O canmount=off \
    -O mountpoint=none \
    -R "$ENV_ZFS_ROOT/$ENV_POOL_NAME_SYS" \
    \
    "$ENV_POOL_NAME_SYS" \
    "$MIRROR" "$@"
...
## First import
zpool export "$ENV_POOL_NAME_SYS"
zpool import -d /dev/disk/by-id "$ENV_POOL_NAME_SYS"
zfs load-key "$ENV_POOL_NAME_SYS"

## Create datasets
bash ./datasets/create-datasets-for-sys.bash

## Done
zfs snapshot "${ENV_POOL_NAME_SYS}@initial"
exit 0
