#!/usr/bin/env bash
#NOTE: This script is a fragment sourced by a parent script. It must work for both Debian and Ubuntu Server. The parent provides `CWD` and has `set e`. `apt` is used over `apt-get` because this is an attended, semi-interactive install.
