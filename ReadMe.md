...
* `install-deb-distro-from-chroot`: Executes a series of "modules" to set up a `.deb`-based distro from `chroot`.
    ...
    * `fs`: Configure filesystems.
        ...
        * `mem-fs`: Sets up various memory-based filesystems, like `/tmp` and swap. **Configures a tiered memory compression scheme with lighter compression for hotter pages and heavier compression for colder pages, thus expanding total effective memory by as much as practical.**
